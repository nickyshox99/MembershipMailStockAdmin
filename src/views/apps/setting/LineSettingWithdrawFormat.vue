  ...
  <b-card>
    ...
    <b-form>
      <b-row>
        <b-col md="12">
          <b-form-group
            label-for="reviewWithdrawMessage"
            label="Review Withdraw Message"
          >          
            <b-form-textarea
              id="textarea"
              v-model="reviewWithdrawMessage"
              placeholder=""
              rows="10"
              max-rows="15"      
              readonly
      ...
      <b-row>
        <b-col md="12">
          <b-form-group
            label-for="wittextsimulate"
            label="Format Text"
          >            
            <b-form-textarea
              id="wittextsimulate"
              v-model="localOptions.wit_textsimulate"
              placeholder=""
              rows="10"
              max-rows="15"
...
export default {
  ...
  methods: {
    ...
    async updateReviewMessage()
    {
      ...
      let tmpMsg = this.localOptions['wit_textsimulate'];
      for (const [key, value] of Object.entries(tmpMsgReplace)) 
      {
        tmpMsg = tmpMsg.replaceAll(key,value);
      }
      this.reviewWithdrawMessage = tmpMsg;

    }    ,
    insertTag(tagname)
    {
      this.localOptions['wit_textsimulate'] += tagname;
      this.updateReviewMessage();
    }
  },
}
