<template>
  ...
  <div>
    ...
    <product-setting-edit :isEditFormActive="isEditFormActive" :pRowData="selectedDataRow" 
...
import ProductSettingEdit from './ProductSettingEdit.vue';
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      const response = await this.GetProductSetting(formData);
      if (response.data.status == 'success') {  
        // console.log(response.data.data);
    ...
    addnew() {
      ...
      let EmptyRow = {
        product_name : '-', 
        product_img : '/images/Netflix.png',
        product_desc : '-',               
        use_credit : 1.00,
    ...
    async deleteProduct(listId) {
      ...
      let response;
      await axios.post("api/product/deleteProduct/", body,
        {
          headers: {
