...
export default {
  ...
  methods: {
    ...
    ...mapActions(["AddPaymentNoteGroup"]), 
    ...
    async insertNoteSub() {
      ...
      let response;
      await axios
        .post("api/subscriptiongroup/addPaymentNoteGroup", body, {
          headers: {
    ...
    async deletePayment(selectID)
    {
        ...
        const userData = JSON.parse(localStorage.getItem('userData'));
            ...
            let response;
            await axios.post("api/subscriptiongroup/deletePaymentHistoryByID",body,
            {
                headers: {            
                'Content-Type': 'application/json',
                'userid': headers.userid,
