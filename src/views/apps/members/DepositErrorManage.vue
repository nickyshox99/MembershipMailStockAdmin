<template>
  ...
  <div>
      ...
      <b-card v-if="!(isDepositFormActive)&&!(isWithdrawFormActive)">
        <div class="m-2">
          ...
          <b-row style="justify-content: space-between!important;">
            <b-col cols="12" md="8" class="d-flex align-items-center justify-content-start mb-1">
              <b-button variant="danger" @click="rejectAllConfirm">
              <feather-icon
                icon="TrashIcon"              
              />            
              </b-button>
    ...
    <Transition name="fade" mode="out-in">
      <b-card :title="t('รายการฝาก (ผิดพลาด)')" v-if="!(isDepositFormActive)&&!(isWithdrawFormActive)">
        <vue-good-table ref="my-table" :columns="columns" :rows="rows" :rtl="direction" :line-numbers="true" 
          ...
          <template slot="table-row" slot-scope="props" >
            ...
            <div v-if="props.column.field === 'calculate'">              
                <h6 class="text-success" >{{ parseFloat(props.row.credit).toFixed(2) }}</h6>
            </div>
            ...
            <div v-if="props.column.field === 'bankinfo2'">
              <b-media vertical-align="center">            
                ...
                <b-badge
                    pill
                    :variant="resolveStatusVariant(2)"
            ...
            <div v-if="props.column.field === 'action'" >
                <b-dropdown variant="outline-success" toggle-class="text-decoration-none" no-caret size="sm" class="lang-dropdown" >
                  ...
                  <b-dropdown-item @click="approveModal(props.row)" v-if="props.row.transaction_type=='DEPNL'" >
                      <feather-icon icon="CheckSquareIcon" class="mr-50" />
                      <span>{{t('Approve')}}</span>
                  </b-dropdown-item>

                  <b-dropdown-item @click="updateApproveData(props.row,'approve',0)" v-if="(props.row.transaction_type=='DEPERR' ||  props.row.transaction_type=='DEPMIN')"  >
                      <feather-icon icon="CheckSquareIcon" class="mr-50" />
                      <span>{{t('Approve')}}</span>
                  </b-dropdown-item>

                  <b-dropdown-item @click="updateApproveData(props.row,'approve',1)" v-if="(props.row.transaction_type=='DEPERR' ||  props.row.transaction_type=='DEPMIN')">
                    <feather-icon icon="CheckSquareIcon" class="mr-50" />
                    <span>{{t('Approve (Turn On Auto Bank)')}}</span>
                  </b-dropdown-item>
               
                  <b-dropdown-item @click="approveModal(props.row)" v-if="props.row.transaction_type=='DEPMAN'" >
                      <feather-icon icon="CheckSquareIcon" class="mr-50" />
                      <span>{{t('Approve')}}</span>
                  </b-dropdown-item>
                ...
                <b-dropdown variant="outline-danger" toggle-class="text-decoration-none" no-caret size="sm" class="lang-dropdown" >
                  ...
                  <b-dropdown-item @click="updateApproveData(props.row,'cancel',0)">
    ...
    <Transition name="fade" mode="out-in">
      <b-modal ref="bv-modal-approve" hide-footer>
        <template #modal-title>
          {{t('Approve Deposit Error')}}
        </template>
        <div class="d-block text-center">
          <h5>Please Enter Member Tel No.</h5>
          <b-form-group label-for="telno">
              <b-input-group class="input-group-merge">
                  <b-input-group-prepend is-text>
                      <feather-icon icon="SmartphoneIcon" />
                  </b-input-group-prepend>
                  <b-form-input id="telno" v-model="selectedUsername" placeholder="0988881234"  />
              </b-input-group>
          </b-form-group>
          <hr/>
          <h6 class="text-danger" > {{ withdrawerror }}</h6>
          <b-button type="submit" variant="primary" class="mr-1" @click="approveConfirm()"  >
...
export default {
  ...
  methods: {
    ...
    modalclose()
    {
      this.$refs['bv-modal-approve'].hide();      
      this.acting=false;
    },
    async approveModal(row)
    {
      this.acting=true;
      this.approveRow=row;     
      this.selectedUsername=""; 
      this.$refs['bv-modal-approve'].show();
    },
    async approveConfirm()
    {
      ...
      body.mode = "approve";
      body.autobank = 0;
      body.selectedUsername = this.selectedUsername;

      let response;
      await axios.post("api/transaction/updatewaitdeposittransactionbyid", body, {
          headers: {
              'Content-Type': 'application/json',
              'userid': headers.userid,
              'token': headers.token,
    ...
    async updateApproveData(row,mode,autobank)
    {
      console.log("updateApproveData");
    ...
    async rejectAllConfirm()
    {
      ...
      {
        this.boxTwo = '';
        await this.$bvModal.msgBoxConfirm('Please confirm that you want to reject.', {
          title: this.$t('Please Confirm'),
          size: 'sm',
          buttonSize: 'sm',
          okVariant: 'danger',
        ...
        .then(value => {              
          ...
          { 
            ...
            for (let index = 0; index < selectedRow.length; index++) {
              ...
              //   autobank : 0,
              // }
              let element = selectedRow[index];
              this.updateApproveData(element,'cancel',0);
            }
