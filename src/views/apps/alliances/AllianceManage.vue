<template>
  ...
  <div>
    ...
    <alliance-manage-add :isAddFormActive="isAddFormActive" :pRowData="selectedDataRow"
      @refetch-data="search" @close-edit-form="closeEditForm" />

    <alliance-manage-edit :isEditFormActive="isEditFormActive" :pRowData="selectedDataRow" :isModeEdit="isModeEdit"
      @refetch-data="search" @close-edit-form="closeEditForm" />
    ...
    <Transition name="fade" mode="out-in">
      <b-card title="Manage Alliance" v-if="!(isEditFormActive)&&!(isAddFormActive)">
        <!-- table -->
...
import AllianceManageEdit from './AllianceManageEdit.vue';
import AllianceManageAdd from './AllianceManageAdd.vue';
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      let response;
      await axios.post("api/alliance/getalliance",body,
      {
          headers: {            
        ...
        await this.$bvModal.msgBoxConfirm(this.$t('Please confirm that you want to delete'), {
          ...
          .then(value => {
            ...
            if (value) {
              ...
              this.inactiveAlliance(selectID);
    ...
    async inactiveAlliance(listId) {
      ...
      let response;
      await axios.post("api/alliance/inactivealliancebyid/", body,
        {
          headers: {
            'Content-Type': 'application/json',
            'userid': headers.userid,
