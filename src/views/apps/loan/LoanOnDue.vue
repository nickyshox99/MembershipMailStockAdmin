<template>  
  ...
  <div>
    ...
    <b-card :title="t('Payment On Due Date')">
                ...
                <div v-if="props.column.field === 'id2'">
                  ...
                  <span v-if="props.column.field === 'payment_status2'">
                      ...
                      <b-badge
                          ...
                          class="text-capitalize"
                          v-if="props.row.paid==0 && props.row.overdue==1"
                      >
                          {{ t('Not Paid')}}
                      </b-badge>
                      ...
                      <b-badge
                          ...
                          class="text-capitalize"
                          v-if="props.row.paid==0 && props.row.overdue==0"
                      >
                          {{ t('Not Paid')}}
                      </b-badge>
                  ...
                  <span v-if="props.column.field === 'action'">   
                    ...
                    <b-badge style="cursor: pointer; margin-right:2px; width: 160px; text-align: left;" variant="warning" @click="showKeyFineInfo(props.row)"
                      v-if="pagePermission.canApprove && props.row.overdue==1"
                    >
                        <feather-icon icon="SearchIcon" size="16" class="mr-0 mr-sm-50" />
                    ...
                    <b-badge style="cursor: pointer; margin-right:2px; width: 160px; text-align: left;" variant="success" @click="showKeyPamentInfo(props.row)"
                      v-if="pagePermission.canEdit && props.row.overdue==1"
                    >
                    ...
                    v-if="pagePermission.canApprove && props.row.paid!=1 && props.row.closed!=1 && props.row.rejected!=1 && props.row.overdue==1"
                    >
...
import { entries, forEach } from 'postcss-rtl/lib/affected-props';
...
export default {
  ...
  methods: {    
    ...
    async search(){
      ...
      formData.append("startDate", this.formatDateAssigned2(new Date()));
      formData.append("endDate", this.formatDateAssigned2(new Date()));
      
      formData.append("admin_id", userData.username);
      formData.append("page_name", this.$route.name);
      
      const response = await this.GetLoanOnDueDate(formData);
