<template>
  ...
  <div>
    ...
    <loan-type-edit :isEditFormActive="isEditFormActive" :pRowData="selectedDataRow" 
    ...
    <Transition name="fade" mode="out-in">
      <b-card :title="t('Loan Type')" v-if="!(isEditFormActive)">
        <!-- table -->
        <vue-good-table ref="my-table" :columns="columns" :rows="rows" :rtl="direction" :line-numbers="true"
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <span v-if="props.column.field === 'period_name2'">              
                  {{t(props.row.period_name)}}
            </span>

            <span v-if="props.column.field === 'collateral_name2'">              
                  {{t(props.row.collateral_name)}}
            </span>

            <span v-if="props.column.field === 'period_unit2'">              
                  {{t(props.row.period_unit)}}
            </span>
...
import LoanTypeEdit from './LoanTypeEdit.vue';
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      const response = await this.GetInterestType(formData);
      if (response.data.status == 'success') {  
        // console.log(response.data.data);
    ...
    async deletePromotion(listId) {
      ...
      let response;
      await axios.post("api/interesttype/deleteById/", body,
        {
          headers: {
