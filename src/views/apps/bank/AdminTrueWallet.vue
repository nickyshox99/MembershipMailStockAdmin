<template>
  ...
  <div>
    ...
    <admin-true-wallet-edit :isEditFormActive="isEditFormActive" :pRowData="selectedDataRow" :isModeEdit="isModeEdit"
    ...
    <Transition name="fade" mode="out-in">
      <b-card title="จัดการทรูวอเรส" v-if="!(isEditFormActive)">
        <!-- table -->
...
import AdminTrueWalletEdit from './AdminTrueWalletEdit.vue';
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      const response = await this.GetAdminTrueWallet(formData);
        ...
        await this.$bvModal.msgBoxConfirm(this.$t('Please confirm that you want to delete'), {
          ...
          .then(value => {
            ...
            if (value) {
              ...
              this.deleteAdminTrueWallet(selectID);
    ...
    async deleteAdminTrueWallet(listId) {
      //const passwordCrypted = bcrypt.hash(user.get("password"),saltRounds);

      console.log("deleteAdminTrueWallet");
      ...
      let response;
      await axios.post("api/admintruewallet/deleteadmintruewalletbyid/", body,
        {
          headers: {
