<template>
  ...
  <div>
    ...
    <staff-setting-edit :isEditFormActive="isEditFormActive" :pRowData="selectedDataRow" 
    ...
    <Transition name="fade" mode="out-in">
      <b-card title="จัดการพนักงาน" v-if="!(isEditFormActive)">
        <!-- table -->
        <vue-good-table ref="my-table" :columns="columns" :rows="rows" :rtl="direction" :line-numbers="true"
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <span v-if="props.column.field === 'am_rank2'">              
                <b-badge
                    pill
                    :variant="resolveStatusVariant2(props.row.am_rank)"
                    class="text-capitalize"
                >
                    {{t(resolveStatusText2(props.row.am_rank))}}
                </b-badge>
            </span>
...
import StaffSettingEdit from './StaffSettingEdit.vue';
...
export default {
  ...
  methods: {        
    ...
    async search() {
      ...
      const response = await this.GetStaff(formData);
    ...
    async deleteStaff(listId) {
      ...
      let response;
      await axios.post("api/staff/deletestaffbyid/", body,
        {
          headers: {
