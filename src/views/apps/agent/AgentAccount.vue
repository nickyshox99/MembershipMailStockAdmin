<template>  
  ...
  <div>
    ...
    <Transition name="fade" mode="out-in" >
      <b-card title="จัดการเอเจนท์" v-if="!(isEditFormActive)"  >
        <!-- table -->
        <vue-good-table
          ref="my-table"
          :columns="columns"
...
import AgentAccountEdit from './AgentAccountEdit.vue';
...
export default {
  ...
  setup() {
    ...
    let columns = [
        ...
        {
          label: t('End Point API'),
          field: 'end_point_api',
        },
        {
          label: t('End Point Game'),
          field: 'end_point_game',
        },  
  ...
  methods: {   
   ...
   async search(){
      ...
      const response = await this.GetAgent(formData);
      ...
      if (selectedRow.length>0) 
      {
        ...
        .then(value => {              
          ...
          { 
            ...
            this.deleteAgentAccountEdit(selectID);
   ...
   async deleteAgentAccountEdit(listId) {        
          //const passwordCrypted = bcrypt.hash(user.get("password"),saltRounds);

          console.log("deleteAgentAccountEdit");
          ...
          let response;
          await axios.post("api/agent/deleteagentbyid/",body,
          {
              headers: {            
