<template>  
  ...
  <div>
    ...
    <report-invite-friends-deposit-view
    ...
    <b-card :title="t('Bonus From Friend Report')">
      <!-- table -->
      <vue-good-table
        :columns="columns"
        :rows="rows"
      ...
      >
        ...
        >
          ...
          <span v-if="props.column.field === 'username'">
            <b-badge :variant="statusVariant('Professional')">
              {{ props.row.username }}
            </b-badge>
          </span>

          <span v-if="props.column.field === 'mobile_no'">
            <b-badge :variant="statusVariant('Professional')">
              {{ props.row.mobile_no }}
            </b-badge>
          </span>

          <span v-if="props.column.field === 'credit_bonus'">
            <b-badge :variant="statusVariant('Professional')">
              {{ props.row.credit_bonus }}
            </b-badge>
          </span>

          <span v-if="props.column.field === 'aff_id'">
            <b-badge :variant="statusVariant('Applied')">
              {{ props.row.aff_id }}
            </b-badge>
          </span>

          <span v-if="props.column.field === 'mobile_no_aff'">
            <b-badge :variant="statusVariant('Applied')">
              {{ props.row.mobile_no_aff }}
            </b-badge>
          </span>

          <span v-if="props.column.field === 'aff_credit'">
            <b-badge :variant="statusVariant('Applied')">
              {{ props.row.aff_credit }}
            </b-badge>
          </span>
          ...
          <!-- Column: Common -->          
          <span v-if="!['credit_bonus', 'aff_credit','username','mobile_no','mobile_no_aff','aff_id'].includes(props.column.field)">
...
import ReportInviteFriendsDepositView from './ReportInviteFriendsDepositView.vue';
import { useUtils as useI18nUtils } from '@core/libs/i18n'

export default {
  components: {
    // 'report-invite-friends-view':ReportInviteFriendsView,
    ReportInviteFriendsDepositView,
    BRow,
    BCol,    
    VueGoodTable,
    BAvatar,
  ...
  setup()
  {
    ...
    const { t } = useI18nUtils();
      ...
      let columns =  [        
        ...
        {
          label: t('Deposit Username'),
          field: 'aff_id',                 
        },       
        {
          label: t('Deposit Mobile No'),
          field: 'mobile_no_aff',                 
        },       
        {
          label: t('Deposit'),
  ...
  methods: {
   ...mapActions(["GetAgent"]),
   ...mapActions(["GetReportAffDeposit"]),
   async search(){
      console.log('search');
      ...
      const response = await this.GetReportAffDeposit(formData);      
