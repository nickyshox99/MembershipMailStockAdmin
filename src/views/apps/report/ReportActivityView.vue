<template>
    ...
    <b-sidebar 
      ...
      right
      @hidden="resetForm"
      @change="(val) => $emit('update:is-report-activity-view-active', val)"
    >
  ...
  export default {
    ...
    model: {
      ...
      event: 'update:is-report-activity-view-active',
    },    
    props: {
    ...
    setup(props, { emit }) {
      ...
      const onSubmit = () => {
        store.dispatch('app-user/addUser', tmpData)
          .then(() => {
            emit('refetch-data')
            emit('update:is-report-activity-view-active', false)
          })
      }
