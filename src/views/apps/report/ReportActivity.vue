<template>  
  ...
  <div>
    ...
    <report-activity-view
      :is-report-activity-view-active.sync="isAddNewUserSidebarActive" 
    ...
    <b-card :title="t('Activity Report')">
      <!-- table -->
      <vue-good-table
        :columns="columns"
        :rows="rows"
...
import ReportActivityView from './ReportActivityView.vue';

export default {
  components: {
    // 'report-activity-view':ReportActivityView,
    ReportActivityView,
    BRow,
    BCol,    
    VueGoodTable,
  ...
  methods: {
   ...
   async search(){
      ...
      const response = await this.GetReportActivity(formData);
